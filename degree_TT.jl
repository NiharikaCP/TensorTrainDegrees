...
#-----------------------------------
# Uniqueness check: are minimal primes uniquely attained?
#
# Each valid pair (P-family, Q-family) determines a minimal prime of J≻,
# namely the ideal generated by the variables NOT in (∪P_ℓ) ∩ (∪Q_ℓ). Two
# pairs give the same minimal prime iff they give the same intersection
# set. compute_degree simply counts valid pairs; this function additionally
# checks whether that count ever overcounts distinct primes.

"""
    compute_degree_check_uniqueness(d1, d2, d3, r1, r2; verbose=false)

Same computation as `compute_degree`, but also records the intersection
set (∪P_ℓ) ∩ (∪Q_ℓ) for every valid pair and checks whether these sets are
pairwise distinct across all valid pairs.

Returns a named tuple `(degree, n_distinct_primes, injective)` where
`injective == true` means every valid pair gave a different minimal prime
(so `degree == n_distinct_primes`), and `injective == false` means at
least two distinct pairs produced the same minimal prime (so naively
counting pairs would overcount the true degree).
"""
function compute_degree_check_uniqueness(d1::Int, d2::Int, d3::Int, r1::Int, r2::Int; verbose::Bool=false)
    p1, q1 = d1, d2 * d3
    p2, q2 = d1 * d2, d3
    @assert r1 <= min(p1, q1) "r1 out of range"
    @assert r2 <= min(p2, q2) "r2 out of range"
    ...
    deg = 0
    primes = Set{Set{Tuple{Int,Int,Int}}}()
    for s1 in fam1_sets, s2 in fam2_sets
    ...
    n_distinct = length(primes)
    injective = (n_distinct == deg)

    if verbose
        status = injective ? "INJECTIVE (all distinct)" : "NOT injective ($(deg - n_distinct) collisions)"
        println("degree (pairs) = ", deg, ", distinct minimal primes = ", n_distinct, "  [", status, "]")
    end

    return (degree=deg, n_distinct_primes=n_distinct, injective=injective)
end
    return deg
end
